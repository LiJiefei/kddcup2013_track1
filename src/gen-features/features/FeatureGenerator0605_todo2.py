...
class FeatureGenerator0605_todo2:
    ...
    def get_feature(self, authorid, paperid):
        ...
        if len(author_name_set) == 1:
            return [-1]
        
        sum_simi = 0
        l = len(author_name_set)
        author_name_set = list(author_name_set)
        cnt = 0
        for i in range(l-1):
            for j in range(i+1, l):
                sum_simi += nameSimi(author_name_set[i], author_name_set[j])
                cnt += 1
        return [sum_simi/cnt]
